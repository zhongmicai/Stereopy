...
import os
import cv2
from .utils import remove_small, hole_fill
...
def remove_objects(img, dst=None, area_thre=24):
    step = 5000
    overlap = 500
    if dst is None:
        dst = np.zeros(img.shape, dtype=np.uint8)
    dst.fill(0)
    for idx in range(img.shape[0]//step + 1):
        start = max(idx * step - overlap, 0)
        end = min((idx+1) * step + overlap, img.shape[0])
        label = measure.label(img[start:end].astype(np.bool_), connectivity=2)
        props = measure.regionprops(label)
        for p in props:
            if p['area'] >= area_thre:
                bbox_p = p['bbox']
                temp = dst[start+bbox_p[0]: start+bbox_p[2], bbox_p[1]: bbox_p[3]]
                np.logical_or(temp, p['image'], out=temp)
    return dst

def edgeSmooth(img):

    # remove small objects
    img_filter = remove_objects(img, None, 24)

    # hole fill
    np.logical_not(img_filter, out=img_filter)
    remove_objects(img_filter, img, 24)
    np.logical_not(img, out=img_filter)

    # morphology open
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))
    cv2.morphologyEx(img_filter, cv2.MORPH_OPEN, kernel, dst=img)
