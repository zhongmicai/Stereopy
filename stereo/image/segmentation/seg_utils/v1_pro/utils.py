...
def remove_small(img, thre=0.2):
    label = measure.label(img, connectivity=2)
    props = measure.regionprops(label)
    num = len(props)
    # print('connecty num:', num)
    threhold = int(thre * num)
    area_list = [p['area'] for p in props]
    area_list.sort()
    area_thre = area_list[threhold:][0]
    print('area_thre:', area_thre)
    img_filter = np.zeros((img.shape), dtype=np.uint8)
    for idx, obj in enumerate(props):
        if obj['area'] > area_thre:
            bbox_p = obj['bbox']
            img_filter[bbox_p[0]: bbox_p[2], bbox_p[1]: bbox_p[3]] += obj['image']
...
def merge(label_list, x_list, y_list, shapes,  overlap=100, type=np.uint8):
    ...
    for index, temp_img in enumerate(label_list):
        ...
        if overlap == 0:
            ...
            if int(info[0]) == 0 or int(info[1]) == 0:
                image[int(x_begin): int(x_begin) + h - 2, int(y_begin): int(y_begin) + w - 2] = temp_img[1: - 1, 1: - 1]
            else:
                image[int(x_begin): int(x_begin) + h - overlap, int(y_begin): int(y_begin) + w - overlap] = temp_img[
                                                                                                            overlap // 2: - overlap // 2,
                                                                                                            overlap // 2: - overlap // 2]
...
def split_preproc(image, cut_size, overlap=100):
...
def merge_preproc(label_list, x_list, y_list, shapes,  overlap=100, type=np.uint8):

    if len(label_list) == 1:
        return label_list[0].get()
...
def transfer_16bit_to_8bit(image_16bit):
    ...
    for idx in range(image_16bit.shape[0]//CHUNK_SIZE+1):
        ...
        image_8bit[sl] = np.array(np.rint((image_16bit[sl,:]-min_16bit)*div), dtype=np.uint8)
