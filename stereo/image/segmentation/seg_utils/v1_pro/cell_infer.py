...
from .utils import split, merge, view_bar, split_preproc
    ...
    if 0:
        for idx, image in enumerate(file_list):
            ...
            for idy, img in enumerate(img_list):
                ...
                if len(img.shape) != 4:
                    logger.info('input data shape wrong!!')
            ...
            for batch in tqdm(test_dataloader, ncols=80):
                ...
                for i in range(bacth_size):
                    ...
                    if int(info[0]) == 0 or int(info[1]) == 0:
                        x_begin = int(info[0])
                        y_begin = int(info[1])
                        merge_label[int(x_begin): int(x_begin) + h - 2, int(y_begin): int(y_begin) + w - 2] = temp_img[
                                                                                                              1: - 1,
                                                                                                              1: - 1]
                    else:
                        ...
                        int(y_begin): int(y_begin) + w - overlap] = temp_img[
                                                                    overlap // 2: - overlap // 2,
                                                                    overlap // 2: - overlap // 2]
                img_idx += 20
